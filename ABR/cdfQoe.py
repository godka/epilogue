...
def readlog(filename):
    ...
    for _line in _file:
        lines = _line.split(' ')
        if len(lines) >= 3:
            qoe = float(lines[0]) / 1000. - 4.3 * float(lines[1]) -  float(lines[2]) / 1000.
            #if qoe <= -0.5:
                #print qoe
                #qoe = -2 * np.random.uniform()
            _tmpA.append(qoe / 198.)
    _tmpA.append(4.5)
    _file.close()
    ...
    _a_x, _a_y = np.histogram(_tmpA,bins=50)#,range=(-1.0,4.5))
...
ax1.set_xlabel(r'\textbf{Avg.QoE}')
ax1.set_xlim(0.,4.5)
